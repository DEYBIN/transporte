...
func RutasCliente(r *mux.Router) {
	...
	s.Handle("/filter-customer/{n_docu}", middleware.Autentication(http.HandlerFunc(filterCliente))).Methods("GET")
...
func filterCliente(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content Type", "Aplication-Json")
	response := controller.NewResponseManager()
	params := mux.Vars(r)
	n_docu := params["n_docu"]
	if n_docu == "" {
		response.Msg = "Error to filter customer"
